...
from scansor.observation_mapping import assess_nominal_support
...
def _declaration_parameter_policy(
    ...
    parameters = factor_set.declaration.parameters
    return (
        tuple(parameter.parameter_id for parameter in parameters),
        tuple(parameter.lower for parameter in parameters),
        tuple(parameter.upper for parameter in parameters),
        tuple(parameter.diagnostic_scale for parameter in parameters),
    )
...
def create_execution_request(
    ...
    *,
    callback_limit: int = MAX_CALLBACK_ATTEMPTS,
    ...
    if selection.model_id != factor_set.model_id:
        raise ScansorError("active-factor selection belongs to another model")
    ...
    if initial_parameters.model_id != factor_set.model_id:
        raise ScansorError("initial parameters belong to another model")
    order, lower, upper, scales = _declaration_parameter_policy(factor_set)
    if len(order) > MAX_PARAMETER_DIMENSION:
        raise ScansorError(
            f"execution parameter dimension exceeds {MAX_PARAMETER_DIMENSION}"
        )
    if len(initial_parameters.values) != len(order):
        raise ScansorError(
            "initial parameter dimension disagrees with model declaration"
        )
    return _identified(
        ExecutionRequest,
        "execution-request",
...
def _validate_execution_graph(
    ...
    if expected != request:
        raise ScansorError(
            "execution request does not match the revalidated declared factor graph"
        )
...
class _GuardedCallback:
        ...
        with self.state_gate:
            ...
            if self.next_sequence >= self.request.callback_limit:
                ...
                if (
                    ...
                    self._interrupted_sequences[self.active_sequence] = sequence
    ...
    def __call__(self, values: object) -> FactorEvaluation:
        ...
        try:
            ...
            converted_values = tuple(converted)
            ...
            if not _in_bounds(self.request, converted_values):
                return self._reject(
                    sequence_id,
                    converted_values,
            ...
            try:
                _ = validate_runtime_parameter_structure(
                    self.request.declaration, converted_values
                )
            except ScansorError:
                return self._reject(
                    sequence_id,
...
def _structure_is_valid(request: ExecutionRequest, values: tuple[float, ...]) -> bool:
    try:
        _ = validate_runtime_parameter_structure(request.declaration, values)
    except ScansorError:
        return False
    return True
...
def execute(
    ...
    if (
        preflight.model_id != request.model_id
        or preflight.factor_set_id != request.factor_set_id
        or preflight.selection_id != request.selection_id
    ):
        raise ScansorError("declared preflight provenance disagrees with request")
    try:
        ...
        if incomplete_callbacks:
            values.update(
                **_callback_record(callback),
                ...
                failures=callback.failure_codes,
                external_failure_evidence=None,
        ...
        values.update(
            **_callback_record(callback),
            ...
            external_failure_evidence=(
                None
                if callback_failures
                else _external_failure(request.model_id, "adapter-raised")
            ),
            invocation=invocation,
        ...
        values.update(
            **_callback_record(callback),
            ...
            external_failure_evidence=_external_failure(
                request.model_id, "response-schema-invalid"
            ),
            invocation=invocation,
    ...
    if supplied_final_values is not None and len(
        ...
        observed_dimension = len(cast(Any, supplied_final_values))
        values.update(
            **_callback_record(callback),
            disposition="invalid-backend-output",
    ...
    try:
        ...
        values.update(
            **_callback_record(callback),
            ...
            failures=(code,),
            external_failure_evidence=_external_failure(
                request.model_id, cast(Any, code)
            ),
            invocation=invocation,
            normalized_termination=NormalizedTermination(category="invalid-response"),
        )
    ...
    if (
        ...
        or response.model_id != invocation.model_id
        ...
        values.update(
            **_callback_record(callback),
            ...
            external_failure_evidence=_external_failure(
                request.model_id, "response-provenance-invalid"
            ),
            invocation=invocation,
            normalized_termination=NormalizedTermination(category="invalid-response"),
        )
    ...
    if not _in_bounds(request, response.final_values):
        ...
        return _result(values)
    if not _structure_is_valid(request, response.final_values):
        values.update(
            **_callback_record(callback),
            disposition="invalid-backend-output",
    ...
    try:
        ...
        return _result(values)
    try:
        objective = raw_objective(final_evaluation.raw_residuals_m)
    except ValueError:
        values.update(
            **_callback_record(callback),
            disposition="execution-failed",
            failures=("final-objective-nonfinite",),
            invocation=invocation,
            normalized_termination=NormalizedTermination(category="invalid-response"),
            raw_response=response,
        )
...
def replay_execution(
    ...
    if result.request != request or result.model_id != request.model_id:
        raise ScansorError(
            "execution replay request or model differs from recorded result"
        )
    preflight = preflight_factors(factor_set, selection, request.initial_parameters)
    if (
        result.preflight_id != preflight.preflight_id
        or preflight.model_id != request.model_id
    ):
    ...
    for entry in result.callback_trace:
        ...
        if observation in consistency_observations:
            if entry.failure_code == "callback-reentrant":
                ...
                if not _structure_is_valid(request, entry.values):
                    raise ScansorError(
        ...
        elif entry.failure_code in {"callback-reentrant", "callback-nonfinite"}:
    ...
    if result.disposition == "ineligible":
        ...
        if valid:
            ...
            domain_valid = (
                final_parameters.model_id == request.model_id
                and raw_response.model_id == request.model_id
                and _in_bounds(request, final_parameters.values)
                and _structure_is_valid(request, final_parameters.values)
            )
            try:
                final = evaluate_factors(factor_set, selection, final_parameters)
                objective = raw_objective(final.raw_residuals_m)
            except (ScansorError, ValidationError, ValueError, FloatingPointError):
                valid = False
            else:
        ...
        if valid and result.failures == ("response-vector-invalid",):
            if result.raw_response is None:
                ...
                valid = (
                    ...
                    and evidence.model_id == request.model_id
                ...
                valid = (
                    result.external_failure_evidence is None
                    and result.raw_response.model_id == request.model_id
                    and not _in_bounds(request, result.raw_response.final_values)
                )
        ...
        if result.normalized_termination == NormalizedTermination(
            ...
            valid = (
                ...
                and result.external_failure_evidence.model_id == request.model_id
            )
            ...
            if valid:
                ...
                valid = _in_bounds(
                    request, response_parameters.values
                ) and _structure_is_valid(request, response_parameters.values)
                if valid:
                    try:
                        evaluation = evaluate_factors(
                            factor_set, selection, response_parameters
                        )
                    except (
                        ScansorError,
                        ValidationError,
                        ...
                        valid = result.failures == ("final-evaluation-undefined",)
                    else:
                        if result.failures == ("final-objective-nonfinite",):
                            try:
                                _ = raw_objective(evaluation.raw_residuals_m)
                            except ValueError:
                                pass
                            else:
                                valid = False
...
def assess_held_out(
    ...
    if (
        mapping.model_id != request.model_id
        or mapping.request.declaration != request.declaration
    ):
        raise ScansorError("held-out mapping belongs to another declared model")
    ...
    evaluator = DeclaredGeometryEvaluator(
        request.declaration, result.final_parameters.values
    )
    rows: list[HeldOutRowAssessment] = []
    for observation in mapping.held_out_observations:
        support = assess_nominal_support(mapping.request, observation.point_model_m)
        candidates = tuple(
            HeldOutSupportCandidate(
        ...
        if assigned is not None:
            try:
                residual = evaluator.evaluate_fixed_pose_shape(
                    assigned, observation.point_model_m
                ).residual_m
            except (ScansorError, ValidationError, ValueError, FloatingPointError):
