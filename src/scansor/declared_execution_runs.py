...
from scansor.declared_factors import instantiate_factors, select_active_factors
...
def _inspection_replay_raw(mapping: MappingResult) -> bytes | None:
    """Regenerate only the existing generation-owned stepped synthetic revision."""
...
def _manifest(
    ...
    declaration = records.selection.declaration
    values: dict[str, object] = {
        ...
        "model_id": declaration.model_id,
        "problem": declaration.problem.kind,
...
def _references(
    ...
    if (
        mapping_manifest.declaration != mapping.request.declaration
        or mapping_manifest.model_id != mapping.model_id
        or mapping_manifest.mapping_run_id != mapping.mapping_run_id
    ):
        raise ScansorError("mapping manifest model binding is inconsistent")
    return (
        InspectionRunReference(
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            raise ScansorError(
                f"persisted execution-run graph is invalid: {error}"
            ) from error
        expected_content = {
            name: _artifact(data)
            for name, data in artifacts.items()
        ...
        if (
            mapping.model_id != selection.model_id
            or mapping.request.declaration != selection.declaration
            or manifest.mapping.model_id != mapping.model_id
        ):
            raise ScansorError("execution and mapping model bindings disagree")
        factor_set = instantiate_factors(mapping)
