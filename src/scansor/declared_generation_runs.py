...
from scansor.declared_generation import generated_fixture_provenance, prepare_generation
...
from scansor.mapping_models import MappingRequest, MappingResult
...
def _artifacts(prepared: PreparedGeneration) -> dict[str, bytes]:
    ...
    request = prepared.provenance.request
...
def _publish(output: Path, prepared: PreparedGeneration) -> None:
    ...
    try:
        ...
        raise ScansorError(
            "generation output parent must be a non-symlink directory"
        ) from error
...
def _verify_root(run: Path, opened_root: os.stat_result) -> None:
    try:
        current_root = os.stat(run, follow_symlinks=False)
    except OSError as error:
        raise ScansorError("generation run path changed during verification") from error
...
def verify_generation_run_fd(directory_fd: int, run: Path) -> PreparedGeneration:
    ...
    if (
        ...
        or truth.declaration != request.declaration
        or truth.model_id != request.model_id
    ...
    try:
        ...
        raise ScansorError(
            f"persisted generation artifact graph is invalid: {error}"
        ) from error
...
def create_generated_mapping_run(
    output: Path,
    generation_run: Path,
    inspection_run: Path,
    request: MappingRequest,
) -> MappingResult:
    """Admit a read-only verified declared generation graph before mapping."""
    generation_fd = open_run_directory(generation_run)
    try:
        root = os.fstat(generation_fd)
        generated = verify_generation_run_fd(generation_fd, generation_run)
        expected = generated_fixture_provenance(
            generated, request.input_revision.canonical_sha256
        )
        if request.input_revision.synthetic_fixture != expected:
            raise ScansorError("generation and mapping provenance do not match")

        def verify_generation_input() -> None:
            if verify_generation_run_fd(generation_fd, generation_run) != generated:
                raise ScansorError(
                    "generation artifacts changed during mapping publication"
                )

        return create_mapping_run(
            output,
            inspection_run,
            request,
            generation_identity=(root.st_dev, root.st_ino),
