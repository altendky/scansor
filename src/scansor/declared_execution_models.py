...
from scansor.declared_factor_models import FactorEvaluation, ParameterVector
...
MAX_PARAMETER_DIMENSION = 64
MAX_CALLBACK_ATTEMPTS = 10_000
MAX_CALLBACK_TRACE_BYTES = 16 * 1024 * 1024

EXECUTION_FORMAT = "scansor-declared-analytic-model-execution-result-v1"
HELD_OUT_FORMAT = "scansor-declared-analytic-model-held-out-assessment-v1"
EXECUTION_FORMAT_STATUS = (
...
def raw_objective(residuals: tuple[float, ...]) -> float:
    """Preserve ordinary sums while avoiding intermediate square/sum overflow."""
    try:
        objective = 0.5 * math.fsum(value * value for value in residuals)
    except OverflowError:
        objective = math.inf
    if not math.isfinite(objective):
        norm = math.hypot(*residuals)
        objective = (0.5 * norm) * norm
    if not math.isfinite(objective):
        raise ValueError("raw objective is not representable as a finite float")
    return objective
...
def _validated_declaration(declaration: ModelDeclaration) -> ModelDeclaration:
    try:
        validated = revalidate_model_declaration(declaration)
    except ScansorError as error:
        raise ValueError(f"invalid execution model declaration: {error}") from error
    if validated != declaration:
        raise ValueError("execution model declaration did not revalidate exactly")
    return validated


def _declaration_parameter_policy(
    declaration: ModelDeclaration,
) -> tuple[tuple[str, ...], tuple[float, ...], tuple[float, ...], tuple[float, ...]]:
    return (
        tuple(parameter.parameter_id for parameter in declaration.parameters),
        tuple(parameter.lower for parameter in declaration.parameters),
        tuple(parameter.upper for parameter in declaration.parameters),
...
class AdapterDescriptor(ExecutionStrictModel):
    ...
    protocol_revision: Literal["declared-analytic-model-backend-adapter-v1"] = (
...
class ExecutionRequest(ExecutionStrictModel):
    ...
    callback_protocol_revision: Literal["declared-analytic-model-callback-v1"] = (
        "declared-analytic-model-callback-v1"
    )
    ...
    def validate_request(self) -> ExecutionRequest:
        ...
        order, lower, upper, scales = _declaration_parameter_policy(declaration)
        dimension = len(order)
        if dimension > MAX_PARAMETER_DIMENSION:
            raise ValueError(
                f"execution parameter dimension exceeds {MAX_PARAMETER_DIMENSION}"
            )
        if (
            self.model_id != declaration.model_id
            or self.initial_parameters.model_id != self.model_id
            or len(self.initial_parameters.values) != dimension
            or self.problem != declaration.problem.kind
            or self.parameter_order != order
            or self.lower_bounds != lower
            or self.upper_bounds != upper
            or self.parameter_scales != scales
        ):
            raise ValueError(
                "execution request dimensions, declaration policy, or model binding disagree"
            )
...
class AdapterInvocation(ExecutionStrictModel):
    ...
    mapping_run_id: str = Field(pattern=r"^[0-9a-f]{64}$")
    model_id: str = Field(pattern=r"^model\.[0-9a-f]{64}$")
    parameter_dimension: int = Field(ge=1, le=MAX_PARAMETER_DIMENSION)
    parameter_order: tuple[str, ...]
    parameter_scales: tuple[float, ...]
    problem: Literal["fixed-pose-shape"] = "fixed-pose-shape"
    request_id: str = Field(pattern=r"^execution-request\.[0-9a-f]{64}$")
    ...
    def validate_invocation(self) -> AdapterInvocation:
        ...
        if (
            self.model_id != declaration.model_id
            or self.problem != declaration.problem.kind
            or dimension != len(declaration.parameters)
            or any(
                len(value) != dimension
                for value in (
                    self.initial_values,
            ...
            raise ValueError(
                "adapter invocation dimensions, declaration policy, or model binding disagree"
            )
...
class CallbackTraceEntry(ExecutionStrictModel):
    ...
    def validate_entry(self) -> CallbackTraceEntry:
        ...
        if self.evaluation is not None and (
            ...
            or self.evaluation.model_id != self.model_id
            or self.evaluation.parameters.model_id != self.model_id
            or self.supplied_dimension != len(self.values)
        ):
            raise ValueError(
                "successful callback values or model disagree with evaluation"
            )
...
class CallbackConsistencyObservation(ExecutionStrictModel):
    ...
    related_sequence_id: int | None = Field(
        default=None, ge=0, le=MAX_CALLBACK_ATTEMPTS
    )
    sequence_id: int = Field(ge=0, le=MAX_CALLBACK_ATTEMPTS)
...
class CallbackConsistencyEvidence(ExecutionStrictModel):
    ...
    def validate_evidence(self) -> CallbackConsistencyEvidence:
        ...
        for item in self.observations:
            if item.code == "callback-evaluation-undefined":
                if item.related_sequence_id is None or not any(
                    (item.related_sequence_id, cause) in observations
                    for cause in ("callback-reentrant", "callback-limit-exceeded")
                ):
                    raise ValueError(
                        "evaluation consistency evidence lacks an interruption cause"
                    )
...
class ExecutionResult(ExecutionStrictModel):
    ...
    format: Literal["scansor-declared-analytic-model-execution-result-v1"] = (
        EXECUTION_FORMAT
    )
    ...
    def validate_result(self) -> ExecutionResult:
        ...
        if (
            ...
            or self.model_id != self.request.model_id
            or self.selection_id != self.request.selection_id
            or any(entry.model_id != self.model_id for entry in self.callback_trace)
        ):
            raise ValueError("execution result provenance disagrees with request")
        if self.initial_evaluation is not None and (
            self.initial_evaluation.model_id != self.model_id
            or self.initial_evaluation.parameters.model_id != self.model_id
        ):
            raise ValueError("initial evaluation model binding disagrees")
        if self.raw_response is not None and (
            self.invocation is None
            or (
                self.raw_response.adapter_id != self.adapter_id
                or self.raw_response.model_id != self.model_id
        ...
        if self.invocation is not None:
            ...
            if (
                ...
                or invocation.declaration != request.declaration
                ...
                or invocation.model_id != request.model_id
        ...
        if self.disposition == "completed-not-assessed":
            ...
            assert self.final_parameters is not None
            assert self.final_evaluation is not None
            if (
                self.final_parameters.model_id != self.model_id
                or self.final_evaluation.model_id != self.model_id
                or self.final_evaluation.parameters != self.final_parameters
            ):
                raise ValueError("completed result final model binding disagrees")
        ...
        for entry in self.callback_trace:
            ...
            mismatch = (
                ...
                or evaluation.model_id != self.model_id
                or evaluation.parameters.model_id != self.model_id
        ...
        consistency_trace_codes = {
            ...
            for entry in self.callback_trace
            if entry.failure_code in {"callback-reentrant", "callback-nonfinite"}
        }
        ...
        terminal_sequences = {
            sequence
            for sequence, code in evidence_observations
            if code in {"callback-incomplete", "callback-trace-budget-exceeded"}
        }
        ...
        for sequence, code in evidence_observations:
            ...
            elif code in {"callback-trace-budget-exceeded", "callback-incomplete"}:
                valid_observation = (
        ...
        if self.callback_consistency_evidence is not None and (
            self.callback_consistency_evidence.model_id != self.model_id
            or self.callback_consistency_evidence.retained_trace_byte_count
        ...
        if self.external_failure_evidence is not None:
            ...
            if (
                self.external_failure_evidence.model_id != self.model_id
                or self.failures != (evidence_kind,)
            ):
...
class HeldOutRowAssessment(ExecutionStrictModel):
    assigned_element_id: Identifier | None
    ...
    def validate_row(self) -> HeldOutRowAssessment:
        ...
        if self.assigned_element_id is not None and (
            not self.candidates
            or self.candidates[0].element_id != self.assigned_element_id
        ):
            raise ValueError("held-out assigned element disagrees with candidates")
...
def summarize_held_out(residuals: tuple[float, ...]) -> HeldOutSummary:
    """Aggregate finite residuals without overflowing a representable mean/RMS."""
    ...
    if residuals:
        ...
        try:
            ...
            mean = (
                math.fsum(value / scale for value in residuals) / len(residuals)
            ) * scale
        try:
            rms = math.sqrt(
...
class HeldOutAssessment(ExecutionStrictModel):
    ...
    factor_set_id: str = Field(pattern=r"^factor-set\.[0-9a-f]{64}$")
    format: Literal["scansor-declared-analytic-model-held-out-assessment-v1"] = (
        HELD_OUT_FORMAT
    )
    format_status: Literal[
        "internal/provisional/synthetic-only/compatibility-free/non-public-contract"
    ...
    def validate_assessment(self) -> HeldOutAssessment:
        ...
        if any(row.model_id != self.model_id for row in self.rows):
            raise ValueError("held-out assessment rows mix model identities")
        if tuple(row.row_index for row in self.rows) != tuple(
            sorted(row.row_index for row in self.rows)
        ):
            raise ValueError("held-out assessment rows are not in canonical row order")
        residuals = tuple(
            row.raw_residual_m
            for row in self.rows
            if row.outcome == "assigned" and row.raw_residual_m is not None
        )
        expected_summary = summarize_held_out(residuals)
