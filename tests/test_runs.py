...
from scansor.files import hash_run_file, read_regular, rename_no_replace, write_new_file
...
def settings(
    *, max_vertices: int = 5_000_000, **sources: SettingSource
) -> ResolvedSettings:
...
def test_source_and_output_locations_do_not_change_run_id(tmp_path: Path) -> None:
    ...
    _ = second_source.write_bytes(first_source.read_bytes())
    resolved = settings()
...
def test_inspect_job_rejects_invalid_local_paths(
    ...
    with pytest.raises(ValueError, match="job path"):
        _ = InspectJobConfig.model_validate(values)
...
def test_job_record_fit_options_are_immutable(tmp_path: Path) -> None:
    ...
    with pytest.raises(AttributeError):
        cast(list[str], cast(object, job.supported_fit_options)).append("unsupported")
...
def test_replacement_input_must_replay_identically(tmp_path: Path) -> None:
    ...
    _ = replacement.write_bytes(replacement.read_bytes() + b"x")
    with pytest.raises(ScansorError, match="trailing bytes"):
        _ = verify_run(run, replacement)
...
def test_refuses_overwrite_and_preserves_failed_stage(
    ...
    _ = marker.write_text("preserve", encoding="ascii")
    ...
    def fail_write(_directory_fd: int, _name: str, _data: bytes) -> os.stat_result:
...
def test_failed_publication_preserves_replaced_staging_path(
    ...
    def replace_stage_then_fail(
        _directory_fd: int, _name: str, _data: bytes
    ) -> os.stat_result:
...
def test_failed_publication_preserves_partial_created_file(
    ...
    def partial_write(directory_fd: int, name: str, _data: bytes):
        descriptor = os.open(
            name, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600, dir_fd=directory_fd
        )
        with os.fdopen(descriptor, "wb") as stream:
            _ = stream.write(b"partial")
...
def test_publication_never_deletes_concurrent_output_replacement(
    ...
    def replace_output(
        ...
        original(source_directory_fd, source_name, target_directory_fd, target_name)
        _ = output.rename(moved_published)
        output.mkdir()
        _ = (output / "concurrent-marker").write_text("preserve", encoding="ascii")
...
def test_publication_revalidates_artifacts_after_rename(
    ...
    def mutate_published_run(
        ...
        original(source_directory_fd, source_name, target_directory_fd, target_name)
        if mutation == "add":
            _ = (output / "unexpected").write_bytes(b"race")
        else:
            moved = tmp_path / "moved-report.json"
            _ = (output / "report.json").rename(moved)
            _ = (output / "report.json").write_bytes(moved.read_bytes())
...
def test_publication_setup_failures_close_descriptors(
    ...
    with monkeypatch.context() as context:
        context.setattr(os, "open", tracking_open)
        context.setattr(os, "stat", failing_stat)
        context.setattr(os, "mkdir", failing_mkdir)
        context.setattr(os, "fstat", failing_fstat)
...
def test_symlink_source_parent_and_special_file_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ScansorError, match="non-symlink"):
        _ = inspect_source(request, settings())
...
def test_read_regular_rejects_path_replacement_during_read(
    ...
    def replace_path() -> None:
        if replacement == "file":
            _ = source.rename(tmp_path / "moved-input.ply")
            _ = source.write_bytes(source_bytes)
        else:
            _ = parent.rename(tmp_path / "moved-source")
            parent.mkdir()
            _ = source.write_bytes(source_bytes)
    ...
    class ReplacingStream:
        ...
        def __exit__(
            ...
            _ = self.stream.__exit__(exc_type, exc_value, traceback)
    ...
    def replacing_fdopen(descriptor: int, mode: str) -> ReplacingStream:
        return ReplacingStream(cast(BinaryIO, real_fdopen(descriptor, mode)))

    monkeypatch.setattr(os, "fdopen", replacing_fdopen)
    with pytest.raises(ScansorError, match="changed while reading"):
        _ = files_module.read_regular(source, "test input", len(source_bytes))
    assert replaced
...
def test_verification_rejects_run_root_replacement(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _, run, _resolved = make_run(tmp_path)
...
def test_verification_revalidates_anchored_run_after_replay(
    ...
    def mutate_run_after_source_read(path: Path, label: str, max_bytes: int) -> bytes:
        result = original(path, label, max_bytes)
        if mutation == "root":
            _ = run.rename(moved_run)
            run.mkdir()
        elif mutation == "add":
            _ = (run / "unexpected").write_bytes(b"race")
        else:
            report_path = run / "report.json"
            _ = report_path.rename(tmp_path / "moved-report.json")
            _ = report_path.write_bytes(b"replacement")
        return result
...
def rewrite_run_settings(run: Path, field: str, value: object) -> None:
...
def rewrite_run_source_path(run: Path, value: str) -> None:
    report = parse_canonical_json(
        (run / "report.json").read_bytes(), "report", 8 * 1024 * 1024
    )
...
def rewrite_run_canonical(run: Path, data: bytes) -> None:
    report = parse_canonical_json(
        (run / "report.json").read_bytes(), "report", 8 * 1024 * 1024
    )
    report["canonical"] = {
    ...
    _ = (run / "canonical.npy").write_bytes(data)
    _write_rewritten_report(run, report)
    manifest_path = run / "manifest.json"
    manifest = parse_canonical_json(
        manifest_path.read_bytes(), "manifest", 8 * 1024 * 1024
...
def test_integrity_consistent_wrong_setting_types_fail_cleanly(
    ...
    _, run, _resolved = make_run(tmp_path)
    rewrite_run_settings(run, field, value)
...
def test_integrity_consistent_invalid_source_paths_fail_cleanly(
    ...
    _, run, _resolved = make_run(tmp_path)
    rewrite_run_source_path(run, path)
    with pytest.raises(ScansorError, match="persisted run model is invalid"):
        _ = verify_run(run, None)
