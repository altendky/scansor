...
class NozzleWorkspace:
    ...
    def fit(self, session: NozzleSession) -> SessionFit:
        ...
        result = fit_cone_plane(
            cone, plane, cw, pw, np.array([*baseline["parameters"], 0.0]), self.support
        )
        p = np.array(result["parameters"])
        residual, _ = cone_plane_residual_jacobian(cone, plane, p, self.support)
        axis = np.array([p[2], p[3], 1.0])
        axis /= np.linalg.norm(axis)
        point = np.array([p[0], p[1], 0.0])
        plane_z = float(p[5] - axis @ point)
        if not self.support[0] <= plane_z <= self.support[1]:
