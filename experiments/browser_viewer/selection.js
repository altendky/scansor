...
export function selectionProjection(clip, indices, width, height, occlusion = true) {
  const screen = p => [(p[0] / p[3] + 1) * width / 2, (1 - p[1] / p[3]) * height / 2, p[2] / p[3]];
  const planes = [p => p[3] + p[0], p => p[3] - p[0], p => p[3] + p[1], p => p[3] - p[1], p => p[3] + p[2], p => p[3] - p[2]];
  const vertices = Array.from({length: clip.length / 4}, (_, i) => Array.from(clip.subarray(i * 4, i * 4 + 4)));
  const points = vertices.map(p => p[3] > 0 && planes.every(f => f(p) >= 0) ? screen(p) : null);
  const tile = 32, columns = Math.ceil(width / tile), rows = Math.ceil(height / tile), grid = new Map();
  for (let i = 0; occlusion && i < indices.length; i += 3) {
    ...
    for (const distance of planes) {
      ...
      for (let j = 0; j < polygon.length; j++) {
        ...
        if (da >= 0) next.push(a);
        if ((da < 0) !== (db < 0)) { const t = da / (da - db); next.push(a.map((x, k) => x + t * (b[k] - x))); }
      }
      polygon = next;
    ...
    for (let j = 1; j + 1 < polygon.length; j++) {
      ...
      const left = Math.max(0, Math.floor(Math.min(...xs) / tile)), right = Math.min(columns - 1, Math.floor(Math.max(...xs) / tile));
      const top = Math.max(0, Math.floor(Math.min(...ys) / tile)), bottom = Math.min(rows - 1, Math.floor(Math.max(...ys) / tile));
  ...
  function visible(id) {
    ...
    const p = points[id];
    if (!p) return false;
    const key = Math.min(rows - 1, Math.floor(p[1] / tile)) * columns + Math.min(columns - 1, Math.floor(p[0] / tile));
    for (const [a, b, c] of grid.get(key) || []) {
      ...
      if (u >= -1e-8 && v >= -1e-8 && u + v <= 1 + 1e-8 && u * a[2] + v * b[2] + (1 - u - v) * c[2] < p[2] - 1e-9) {
...
export function brushHits(projection, start, end, radius, depth) {
  ...
  projection.points.forEach((p, id) => {
    if (!p) return;
    const t = length2 ? Math.max(0, Math.min(1, ((p[0] - start[0]) * dx + (p[1] - start[1]) * dy) / length2)) : 0;
    if ((p[0] - start[0] - t * dx) ** 2 + (p[1] - start[1] - t * dy) ** 2 <= radius ** 2 && (depth === 'through_all' || projection.visible(id))) hits.push(id);
  });
  return hits;
}
